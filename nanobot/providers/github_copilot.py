...
try:
    ...
    logger.warning("cryptography not installed. Tokens will be stored in plaintext. Install: pip install cryptography")

from nanobot.providers.base import BaseProvider, ProviderConfig, LLMProvider, LLMResponse, ToolCallRequest
...
class GitHubCopilotProvider(LLMProvider):
    ...
    async def start_device_flow(self) -> DeviceFlowResponse:
        ...
        logger.debug(f"Simulating VS Code {self.VSCODE_VERSION} with Copilot {self.COPILOT_VERSION}")
            ...
            if e.response.status_code == 403:
                ...
                try:
                    ...
                    if 'approved clients' in error_msg.lower():
                        ...
                        logger.error("The Copilot internal API only allows official approved clients (VS Code, JetBrains, GitHub CLI)")
    ...
    async def chat(
        ...
        logger.debug(f"Messages (repr): {[{k: repr(v) if k == 'content' else v for k, v in msg.items()} for msg in messages]}")
    ...
    async def _stream_response(self, headers: dict, payload: dict) -> AsyncIterator[str]:
        ...
        async with self._http_client.stream(
            ...
            try:
                ...
                raise Exception(f"Copilot stream error {e.response.status_code}: {error_body}") from e
