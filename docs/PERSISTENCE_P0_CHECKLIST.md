...
- [x] 项目创建成功并写入 `projects` 表
- [x] 项目更新（名称/描述/状态）可持久化
- [x] 项目删除后不可再查询到
- [x] 旧 `projects.json` 可迁移到 SQLite
...
- [x] 业务层仅依赖服务层，不直接写 SQL
- [x] SQLite 实现位于独立模块，接口语义稳定
- [x] 存储失败不会中断主流程（有降级日志）
...
- [x] 连续多轮对话可恢复历史
- [x] 创建项目并读取项目列表
- [x] 子任务执行后状态落库（pending/running/success/failed）
- [x] LLM 调用后 usage 有记录
