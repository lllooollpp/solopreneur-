    ...
    try:
        ...
        if workspace:
            ...
            try:
                ...
                return resolved, f"Error: Access denied - path '{path}' is outside workspace"
        
        return resolved, None
    except Exception as e:
        return Path(), f"Error: Invalid path - {e}"
...
class ReadFileTool(Tool):
    ...
    async def execute(self, path: str, **kwargs: Any) -> str:
        try:
            # 验证路径安全�?
            file_path, error = _validate_path(path, self.workspace)
            if error:
                return error
...
class ListDirTool(Tool):
    ...
    async def execute(self, path: str, **kwargs: Any) -> str:
        try:
            # 验证路径安全�?
            dir_path, error = _validate_path(path, self.workspace)
            if error:
                return error
