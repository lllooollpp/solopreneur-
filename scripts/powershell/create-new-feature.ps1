...
if ($Help) {
    Write-Host "用法: ./create-new-feature.ps1 [-Json] [-ShortName <名称>] [-Number N] <功能描述>"
    Write-Host ""
    Write-Host "选项:"
    Write-Host "  -Json               以 JSON 格式输出"
    Write-Host "  -ShortName <名称>   提供自定义短名称（2-4 个词）用于分支"
...
if (-not $FeatureDescription -or $FeatureDescription.Count -eq 0) {
    Write-Error "用法: ./create-new-feature.ps1 [-Json] [-ShortName <名称>] <功能描述>"
    exit 1
}
